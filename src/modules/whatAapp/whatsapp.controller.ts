...
const automacaoSchema = z.object({
    ...
    nos: z.array(z.record(z.unknown())).default([]),
});
...
export const whatsappController = {
    ...
    async webhook(req: FastifyRequest, reply: FastifyReply) {
        ...
        if (fromMe) return reply.send({ ok: true }); // ignora mensagens enviadas por nós

        const telefone = (key?.remoteJid as string)?.replace('@s.whatsapp.net', '');
        const conteudo = (data?.message as Record<string, unknown>)?.conversation as string
            ?? (data?.message as Record<string, unknown>)?.extendedTextMessage as string
            ?? '[mídia]';

        // Busca o usuário dono da instância pelo telefone cadastrado
        // Por ora, usa o primeiro usuário com essa instância ativa
        // TODO: mapear instância -> user_id quando tiver configurações
        const instanceName = body?.instance as string;

        // Salva a mensagem recebida (sem user_id por enquanto — vira pelo instanceName)
        // Esta lógica será expandida quando o módulo de configurações estiver pronto
    ...
    async iniciarDisparo(req: FastifyRequest, reply: FastifyReply) {
        ...
        // Busca os leads e dispara em background
        let enviados = 0;
        let falhas = 0;
            ...
            try {
                ...
                if (ok) {
                    ...
                    const conversa = await whatsappRepository.findOrCreateConversa(
                        req.user.id, lead.telefone, lead.name
                    );
                    await whatsappRepository.saveMensagem({
                        user_id: req.user.id,
                        conversa_id: conversa.id,
        ...
        // Atualiza o disparo com resultado
